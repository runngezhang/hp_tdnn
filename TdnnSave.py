...
h1 = tf.layers.conv1d(input, h1_filters, h1_kernel_size,padding='same')
...
h2_kernel_size = 3
h2_filters=500
h2=tf.layers.conv1d(h1_relu_renorm,h2_filters,h2_kernel_size,padding='same')
h2_relu_renorm=tf.layers.batch_normalization(tf.nn.relu(h2))

h3_kernel_size = 3
h3_filters=500
h3=tf.layers.conv1d(h2_relu_renorm,h3_filters,h3_kernel_size,padding='same')
h3_relu_renorm=tf.layers.batch_normalization(tf.nn.relu(h3))

h4_kernel_size = 3
h4_filters=500
h4=tf.layers.conv1d(h3_relu_renorm,h4_filters,h4_kernel_size,padding='same')
h4_relu_renorm=tf.layers.batch_normalization(tf.nn.relu(h4))

h5_kernel_size = 3
h5_filters=500
h5=tf.layers.conv1d(h4_relu_renorm,h5_filters,h5_kernel_size,padding='same')
h5_relu_renorm=tf.layers.batch_normalization(tf.nn.relu(h5))

output_kernel_size=3
output_filter=output_dim
output=tf.layers.conv1d(h5_relu_renorm,output_filter,output_kernel_size,padding='same')
...
for j in range(1,7):
    with tf.Session() as sess:
        saver.restore(sess,"./TDNNmodel"+str(j*5))
        lll=0
        for i in range(0,len(distinct)):
            ...
            #print "prediction:"
            lll+=1-sess.run(accuracy, feed_dict={input: TestFeature[i], y: TestTarget[i].eval(session=sess)})
